...
selectServer <- function(dataset, ...) {
  ...
  function(input, output, session) {
    ...
    ## help menu ##
    imagePath <- file.path(system.file(package = "CardinalVis"), "www")
    ...
    observeEvent(input$openGenHelp, {
      showModal(
        modalDialog(
          ...
          br(), br(), 
          tags$img(src=base64enc::dataURI(file = file.path(imagePath, "help_image_one.jpg"), 
                                          mime = "image/jpeg"),
    ...
    observeEvent(input$openMultiHelp, {
      showModal(
        modalDialog(
          ...
          tags$img(src=base64enc::dataURI(file = file.path(imagePath, "help_image_two.jpg"), 
                                          mime = "image/jpeg"),
                   style="display: block; margin-left: auto; margin-right: auto; width: 40%;")
        )
      )
    ...
    observeEvent(input$openReturnHelp, {
      showModal(
        modalDialog(
          ...
          tags$img(src=base64enc::dataURI(file = file.path(imagePath, "help_image_three.jpg"), 
                                          mime = "image/jpeg"),
